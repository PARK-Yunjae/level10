...
public class _06스트림_실습_기본 {
	...
	public static void main(String[] args) {
		...
		itemList.stream()
				.filter(category -> category.getCategory().equals("과자"))
				.forEach(System.out::println);
		
		// 문제 4번 카테고리가 고기인 item 갯수 출력
		System.out.println("=====[ 4 ]=====");
		long cnt = itemList.stream()
				.filter(category -> category.getCategory()
				.equals("고기"))
				.count();
		System.out.println("고기 개수 = " + cnt + "개");
		// 문제 5번 가격이 10000원 이상인 값들의 ArrayList<Item> 만들기
		System.out.println("=====[ 5 ]=====");
		ArrayList<Item> manwonList = (ArrayList<Item>) itemList.stream()
				.filter(price -> price.getPrice() >= 10000)
				.distinct()
				.collect(Collectors.toList());
		manwonList.forEach(System.out::println);
		// 문제 6번 카테고리가 고기인 아이템 이름만 가져와서 ArrayList<String>으로 만들기
		System.out.println("=====[ 6 ]=====");
		ArrayList<String> tempList = (ArrayList<String>) itemList.stream()

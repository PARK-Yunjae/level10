...
interface Damageable{
	default void attackOpUint(Damageable unit, int power) {
		Unit opUnit = (Unit)unit;
		opUnit.hp -= power;
		System.out.println("%s 가 %s -%d 대미지 공격".formatted(this, opUnit, power));
	}
}
...
public class _05인터페이스_실습 {
	public static void main(String[] args) {
		...
		tank.attackOpUint(marine, rd.nextInt(10)+10);
		marine.attackOpUint(tank, rd.nextInt(10)+10);
		tank.attackOpUint(dropShip, rd.nextInt(10)+10);
		scv.attackOpUint(dropShip, rd.nextInt(10)+10);
		scv.repair(dropShip);
